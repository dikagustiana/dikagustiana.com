 ...
 * actually readable, and joints that are big enough to tap.
 */
...
for (const path of ['/', '/about']) {
  test(`${path} at 360px never scrolls sideways — with the full chain, both toggles and a reading open`, async ({ page }) => {
...
test('/about at 1280px draws one wide plate whose names are readable, with every joint reachable and one panel', async ({ page }) => {
  ...
  // A joint's hit area is at least 24px across on screen.
  const hit = await page.locator('[data-id="j-processing-trader"] circle').boundingBox();
