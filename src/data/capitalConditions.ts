...
import type { CapitalCondition } from '@/components/finance/CapitalConditionCard';
...
export const CONDITIONS: CapitalCondition[] = [
  ...
  },
  {
    ...
    case: 'Adaro Energy vs Bukit Asam',
  ...
  },
  {
    ...
    case: 'Asian Development Bank — Indonesia Programs',
  ...
  },
  {
    ...
    case: 'Medco Energi Internasional',
    tension: 'Undeveloped gas block portfolio creates efficiency drag on current ROIC. Market pressure to optimize producing assets conflicts with strategic value of holding undeveloped reserves as transition optionality — particularly as gas positions as bridge fuel in Indonesia\'s energy mix.',
    quantitative: 'Undeveloped reserve carrying cost → Current ROIC drag 100–150bps → Gas optionality value unpriced in base DCF → If transition timeline accelerates, developed reserves command 20–30% price premium → Optionality destroys short-term metrics while protecting long-term capital position',
  ...
  },
  {
    ...
    case: 'GoTo',
    tension: 'Valuation built on exit multiple expansion rather than operational value creation. When multiple compression occurs, underlying business fundamentals insufficient to sustain capital structure. Investors exposed to multiple risk, not business risk.',
  ...
  },
  {
    ...
    case: 'Indonesia Just Energy Transition Partnership — $20B Commitment',
    tension: 'Concessional capital conditionality conflicts with sovereign policy autonomy. Tranching unclear. Grant, concessional loan, and guarantee instruments bundled without structural separation. Deployment risk high without architecture clarity.',
  ...
  },
  {
    ...
    case: 'PLN Coal Fleet Pre-Retirement',
    tension: 'Coal plants carrying book value significantly above economic value under accelerated transition scenarios. Incremental capex still being allocated to legacy assets without pricing transition liability. Balance sheet does not reflect JETP early retirement commitments.',
    quantitative: 'Remaining book value of coal fleet → Economic value under 1.5°C scenario → Transition liability gap unprovisioned → Incremental capex to legacy assets earning return below transition-adjusted hurdle rate',
  ...
  },
  {
    ...
    case: 'Indonesian Peatland and Forestry Carbon Credit Structuring',
    tension: 'Carbon credit revenue stream dependent on verification timelines mismatched against investor liquidity expectations. Additionality risk and permanence risk unpriced. Capital structure not aligned with underlying ecosystem recovery period.',
  ...
  },
  {
    ...
    case: 'Indonesian formal manufacturing sector — capital intensity rising, labor absorption declining',
    tension: 'Manufacturing capex generates output growth but not structural employment. Private capital follows productivity signals, not social transformation logic. The gap between where capital flows and where structural transformation requires it to go widens without sovereign coordination.',
    quantitative: 'Manufacturing employment share declining despite output growth → Labor productivity gap formal vs informal 3–5x → Private capital allocation follows ROIC signal, not structural employment signal → Sovereign coordination required to bridge gap',
    rule: 'Private capital will not self-correct toward structural transformation. Sovereign capital must create the conditions — de-risk the entry, set the performance criteria, build the coordinating institutions — that make private capital movement rational.',
  },
  {
    id: 'derisking-architecture', number: 20, layer: 'structural-transformation',
    title: 'De-risking Architecture Design',
    case: 'Saratoga capital deployment discipline applied to sovereign de-risking design — Danantara mandate',
    tension: 'Sovereign de-risking fails when designed by people who have never sat on the private capital side. Conditionality gets wrong. Monitoring is performative. First-loss sizing miscalibrated. Private capital reads the architecture and stays out. The instrument exists but the design does not work.',
    quantitative: 'First-loss layer sized incorrectly → Private capital mobilization ratio fails to reach 1:4 threshold → Sovereign capital absorbs full risk with no private leverage → De-risking instrument becomes a subsidy → Capital efficiency collapses → Mandate fails at deployment layer',
  ...
  },
  {
    ...
    case: 'Indonesia green transition capital sequencing — geothermal, nickel downstream, JETP coal retirement',
    tension: 'Danantara faces simultaneous capital demands across transition infrastructure, strategic downstream industries, and legacy asset retirement. Without explicit sequencing logic, capital gets allocated by political priority rather than structural return. The interaction effects between these deployment decisions are not modeled. Capital deployed out of sequence destroys optionality in subsequent tranches.',
    quantitative: 'Geothermal concessional stack → unlocks renewable baseload → enables coal retirement → JETP tranche activated → nickel downstream de-risked → EV supply chain anchored → each stage has capital dependency on prior deployment → sequencing error at any stage collapses mobilization ratio downstream',
...
// ── Index table data (display order) ─────────────────────────────────────────

export const INDEX_TABLE = CONDITIONS.map((c) => ({
  number: c.number,
  title: c.title,
  layer: LAYERS.find((l) => l.key === c.layer)?.label ?? c.layer,
