 ...
 * serves, and a door. Same contract as a joint: always a button, chip on
 * hover, focus or the unit lens, panel on click. The chip word is the
 * layer's margin kind where it earns a fee, or its own word where it only
 * sets the terms.
 */
...
export function BandHit({
  ...
  return (
    ...
    >
      ...
      <text x={x + 10} y={y + 20} className="cp-band-t">
        {band.label}
      </text>
      {noteX !== null && band.note && (
        <text x={noteX} y={y + 20} className="cp-band-n">
          {band.note}
        </text>
      )}
      {chip && (
        <g className="cp-hit-chip" aria-hidden="true">
          <rect x={x + width - w - 10} y={y + (height - CHIP_H) / 2} width={w} height={CHIP_H} rx={2} />
          <text x={x + width - 10 - w / 2} y={y + (height - CHIP_H) / 2 + 13} textAnchor="middle">
            {chip}
          </text>
        </g>
      )}
